...
- [ ] **Re-engage API route** `/api/ai/re-engage` - AI scans `talent_pools` against open `jobs`, returns fit suggestions + outreach drafts. Replace the `setTimeout` stub in `re-engage/page.tsx`. [Page exists but stub only]
- [ ] **Pipeline stage updates** - make the kanban interactive. Card action updates `application.status` via server action / API route. Supabase realtime optional. [Pipeline page reads-only]
