...
- AI application note at `/jobs` — clicking "Apply" on a job card calls `POST /api/ai/cover-note` with `{ jobId }`. Route fetches job (title, description, required_skills, location, employer_profiles) + candidate (name, bio, job_title, years_exp, skills, work_experiences) and calls Groq (`generateText`, `maxOutputTokens: 512`) to produce a 3-paragraph cover note. `ApplyButton.tsx` refactored with a `step` state machine (`idle → generating → editing → submitting`). Generating state shows a pulse indicator. Editing state shows an editable textarea pre-filled with the AI note + "Submit application" and "Skip note" buttons. Both paths insert into `applications` with `notes` field populated (or null for skip). "Cancel" resets to idle. Generation failures fall through silently to editing with empty textarea.
- Skill-gap analysis at `/explore` — `explore/page.tsx` (server) fetches `candidate_skills` alongside nodes/edges, flattens to `candidateSkillNames: string[]`, passes to `CareerPathExplorer`. Detail panel partitions `career_edges.skill_gaps` into "You already have" (green `text-success`) vs "You still need" (amber `text-brand`) by case-insensitive match against candidate's skills. "Generate Learning Roadmap" button (hidden if all gaps covered) calls `POST /api/ai/skill-gap` with `{ currentRole, targetRole, missingSkills: need[] }` — only unmet skills sent to AI. Route (`app/api/ai/skill-gap/route.ts`) uses `generateText`, `maxOutputTokens: 800`, JSON fallback regex `/\{[\s\S]*\}/`, returns `{ roadmap: { summary, steps[{ skill, action, resource }], estimatedMonths } }`. Roadmap rendered inline with numbered step cards; "Regenerate ↺" resets state. Roadmap state resets on node change via `useEffect` on `selectedNode?.id`.
- Job fit score (candidate side) at `/jobs` — `app/api/ai/job-fit/route.ts` (POST, auth-guarded). Fetches job (`title`, `required_skills`, `location`) + candidate profile + skills, sends to Groq (`generateText`, `maxOutputTokens: 256`), returns `{ score: 0-100, summary: string }`. `FitScore.tsx` client component calls the API on mount per job card; renders a color-coded pill (≥70 `--success`, 40-69 `--accent`, <40 `--text-muted`) with `tabular-nums` score + inline one-line summary. Skeleton pulses during load; silently hides on error. Rendered in `jobs/page.tsx` above `ApplyButton`, guarded by `candidateProfile` existence.
- Job board search + filters at `/jobs` — `page.tsx` stays a server component (data fetch), derives `allSkills` as `[...new Set(jobs.flatMap(j => j.required_skills ?? []))].sort()` and passes all jobs to `JobsClientView.tsx` (client component). Filters: keyword (title + description contains), location (contains), employment type (Select, single), salary min (RM input, filters `salary_max >= input*1000`, null-salary jobs always pass), skill badges (multi-select toggle, all selected skills must be present). Filtering is a `useMemo` on filter state — no `useEffect`. "Clear" button appears only when any filter is active. Result count line above card list. Empty filtered state shows inline "Clear filters" link.
- AI job description writer at `/employer/jobs/new` — `POST /api/ai/jd-writer` (auth-guarded). Takes `{ title, location, employmentType, skills, roughNotes }`, calls Groq (`generateText`, `maxOutputTokens: 800`), returns `{ description: string }`. The existing description textarea doubles as both rough-notes input and output. "Polish with AI ✦" button appears inline next to the Description label; disabled when textarea is empty or generation is in-flight. On success, replaces textarea content with the polished JD. No new fields added.
